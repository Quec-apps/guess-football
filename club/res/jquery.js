levels = localStorage.foot_club_Level
total = parseInt(foot_player_LevelTotal);

var coins = localStorage.getItem("footballCoins");
...
// function LevelNext() {
// 	coins++;
// 	localStorage.setItem("footballCoins", coins);
// 	document.getElementById("coins").innerHTML = coins;
// }
$('#coin-txt').text("+1");
...
$(document).ready(function () {
	...
	function Append() {
		...
		$(".option").css({
			backgroundColor: '#f95179', color:'#fff'
		});
	...
	$('.skip-img').click(function () {
		if (coins < 10) {
			...
			coins -= 10;
			levels++; localStorage.foot_club_Level = levels;
			localStorage.footballCoins = coins;
			$("#coins").html(coins);
	...
	$('.btn1').click(function () {
		setInterval(() => {
			coins = localStorage.getItem("footballCoins");
			$("#coins").html(coins);
		}, 2000);
	...
	$("#ans").click(function () {
		...
		setTimeout(() => {
			if (giveReward == 1) {
				...
				coins++; localStorage.setItem("footballCoins", coins);
				$("#coins").html(coins);
			}
			$('.finish-con').css({ display: 'flex' });
		}, 400);
		levels++;
		localStorage.foot_club_Level = levels;
	});
	$(".noans").click(function () {
		...
		setTimeout(() => {
			...
			if (coins >= 4) {
				coins -= 4; localStorage.footballCoins = coins;
				$("#coins").html(coins);
			} else if (coins > 0) {
				coins = 0; localStorage.footballCoins = coins;
				$("#coins").html(coins);
			}
		}, 400);
	});
